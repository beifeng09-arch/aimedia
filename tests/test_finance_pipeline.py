...
def test_finance_pipeline_auto_approve(tmp_path):
    orchestrator = MediaOrchestrator(
        root_dir=PROJECT_ROOT,
        output_dir=tmp_path / "outputs",
        log_dir=tmp_path / "logs",
    )
    result = orchestrator.run_request("做一期比特币周报", auto_approve=True)
    ...
    assert (job_dir / "avatar_script.md").exists()
    assert (job_dir / "publish_pack.md").exists()
    assert (job_dir / "skill_runs.yaml").exists()
    assert "比特币" in (job_dir / "publish_pack.md").read_text(encoding="utf-8")
